...
for _, task_name in ipairs({ "build", "run", "debug", "test", "debug_test" }) do
  ...
  vim.keymap.set({ "", "i" }, string.format("<F%d>", key_index + 12), function()
    tasks.set_task_param("auto", task_name, "args")
  end, { noremap = true, desc = string.format("Set args for %s task", task_name) })

  vim.keymap.set({ "", "i" }, string.format("<F%d>", key_index + 48), function()
...
vim.keymap.set({ "", "i" }, "<F18>", function()
  tasks.set_module_param("auto", "build_type")
end, { desc = "Select CMake build type" })
vim.keymap.set({ "", "i" }, "<F54>", function()
