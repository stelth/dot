...
M.list_lsp_capabilities = function()
    ...
    local function available_capabilities(resolved_capabilities)
        ...
        local display_keys = {
            "call_hierarchy", "code_action", "code_lens", "completion",
            "declaration", "document_formatting", "document_highlight",
            "document_range_formatting", "document_symbol", "execute_command",
            "find_references", "goto_definition", "hover", "implementation",
            "rename", "signature_help", "type_definition"
        }
        return vim.tbl_filter(function(key)
            -- keep only the capabilities that are interesting & available
            return vim.tbl_contains(display_keys, key) and
                       resolved_capabilities[key] == true
    ...
    local function make_client_info(client)
        return {
            ...
            "\t" ..
                table.concat(
                    available_capabilities(client.resolved_capabilities or {}),
                    ', ')
        }
    end
