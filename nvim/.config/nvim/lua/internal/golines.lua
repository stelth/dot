...
local check_same = function(tbl1, tbl2)
    ...
    for k, v in ipairs(tbl1) do if v ~= tbl2[k] then return true end end
    return false
end
...
local golines_format = function()
    ...
    uv.read_start(stderr, function(err, data)
        assert(not err, err)
        if data then print("stderr chunk", stderr, data) end
    end)

    uv.shutdown(stdin, function() uv.close(handle, function() end) end)
end
